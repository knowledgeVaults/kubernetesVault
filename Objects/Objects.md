...
* Exists as a cluster-wide API resource that's managed by the control plan and stored in `etcd` via the `API server`
...
* [[kubernetesVault/Objects/Deployments/Deployments]]: Controllers that manage ReplicaSets to deploy, scale, and update stateless Pods

### Configuration

* [[Config Maps]]: Stores non-confidential configuration data as either key-value pairs in plaintext or binary data
* [[kubernetesVault/Objects/Secrets/Secrets]]: Stores encoded sensitive data for secure Pod access
...
* [[kubernetesVault/Objects/Services/Services]]: Abstractions providing stable endpoints and load balancing for dynamic pods
