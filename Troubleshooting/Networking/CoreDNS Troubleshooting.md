...
Modify the [[CoreDNS]] [[Deployments]] to set `allowPrivilegeEscalation` to `true`
```Bash
...
# 2. Verify DNS service endpoints
kubectl get svc kube-dns -n kube-system
kubectl get endpoints kube-dns -n kube-system    # Must have pod IPs listed

# 3. Test resolution from a debug pod
...
If host uses `systemd-resolved`, configure kubelet to use the real resolv.conf:
```Bash
