...
* Worker node components use it for API server communication
...
- cluster:
    ...
    server: https://<API_SERVER_URL>:6443
  name: my-cluster
contexts:
- context:
...
- name: <USERNAME>
  user:
    client-certificate-data: <BASE64_CLIENT_CERT>  # Base64-encoded client cert
    client-key-data: <BASE64_CLIENT_KEY>          # Base64-encoded client key
```
...
* Defines which user accounts can access which cluster
