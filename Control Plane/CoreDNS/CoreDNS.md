...
* Resolves cluster-internal names by watching the [[kube-apiserver]] for services and endpoints
...
CoreDNS is managed via a [[Config Maps]] (`coredns`) in the `kube-system` namespace
...
* Deployed as two Pods for redundancy as part of a replica set within a [[Deployments]] in the kube-system namespace
...
The `Corefile` configures CoreDNS using a plugin-based system

| Plugin | Purpose |
|---|---|
| `kubernetes` | Serves DNS for Kubernetes Services and Pods |
| `forward` | Forwards non-cluster DNS queries to upstream resolvers |
| `errors` | Logs DNS errors |
| `health` | Exposes health endpoint at `:8080/health` |
| `ready` | Readiness endpoint at `:8181/ready` |
| `cache` | Caches DNS responses |
| `reload` | Auto-reloads Corefile on changes |
